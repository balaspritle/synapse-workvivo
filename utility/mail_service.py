...
import sendgrid, os, sys, base64, time, shutil, datetime, requests, json, pytz
...
# def excel_filtering(duration, file_path):
#     print("Triggered {} for a duration of {} days".format(file_path, duration))
...
#     try:
#         df = pd.read_excel(os.path.join(os.getcwd(), file_path), engine='openpyxl', parse_dates=[0])
#         try:
#             df['Date'] = df.apply(date2str, axis=1)
#         except ValueError:
#             df['Date'] = None
...
#         return df
#     except Exception as e:
#         print("Error in {}".format(file_path), e)
#         return None
...
def date2str(row):
    return (datetime.datetime.strptime(row['timestamp'].split('.')[0], '%Y-%m-%dT%H:%M:%S').date())
...
def sendWeeklyChatLogs(duration, user_id_to_email_mapper):
    ...
    if(type(duration) is tuple):
        start_date = duration[0]
        end_date = duration[-1] + datetime.timedelta(days = 1)
        end_date_original = duration[-1]
    else:
        start_date = (pd.Timestamp.today() - pd.Timedelta('{}D'.format(duration))).date()
        end_date = (pd.Timestamp.today() + pd.Timedelta('1D')).date()
        end_date_original = pd.Timestamp.today().date()
        
    timespan = "{}/{}".format(start_date, end_date)
    timespan_original = "{}/{}".format(start_date, end_date_original)
    ...
    try:    
        ...
        # df_week = df[df['Date'] > pd.Timestamp.today() - pd.Timedelta('{}D'.format(duration))]
        df = df.replace('', np.nan)
        df = df.dropna()
    ...
    try:
        ...
        for sender_email_ in config.sender_email:
            ...
            message = Mail(
                ...
                subject = "Iris Bot - Complete Logs",
                html_content =  text
            )
            message.attachment = attachedFile
...
def consolidated_analytics(duration):
    ...
    df1 = df1[['date', 'Time', 'user_id', 'User Email', 'question', 'answer', 'score', 'duration', 'performanceBucket']]
